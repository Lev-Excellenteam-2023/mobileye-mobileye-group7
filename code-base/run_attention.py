...
def convolve_rgb_image( image:np.ndarray, kernel, mode='same' ) -> \
    ...
    red_peaks_x, red_peaks_y = find_light_point(image, red, 100, "r")
    green_peaks_x, green_peaks_y = find_light_point(image, green, 90, "g")
...
def resize_image ( c_image:np.ndarray, ratio: float ):
    pass
...
def find_tfl_lights(c_image: np.ndarray, **kwargs) -> Dict[str, Any]:
    ...
    c_image = resize_image(c_image,ratio=0.25)
